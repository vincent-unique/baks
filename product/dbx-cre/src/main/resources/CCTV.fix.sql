...
--AFD 在CRE中无法对应AFDTYPE ,建库脚本中新增了 AFD字段做硬同步，赞不做处理

--2、DDM_CLASS (DDM_CLASS)
-- 两个库中 TYPE 都为给出明确定义，暂不处理

--3、DDM_LIMITEDWORD (DDM_LIMITEDWORD)
--两个库中 TYPE 定义一致，不需要转义处理

--4、EXC_EXCHANGEATTRIBUTE (EXC_EXCHANGEATTRIBUTE)
-- 两个库中 TYPE定义一致，不需要转义处理

--5、MSM_ATTRIBUTE （MSM_ATTRIBUTE）
--老媒资中的 TYPE定义 被CRE包含，暂不需要处理
...
--两库中 VIDEOTYPE 定义一致，不需要转义处理
--两库中 AUDIOTYPE 定义一致，不需要转义处理
--两库中 STATUS 定义一致，不需要转义处理

--12、SPM_STORAGEAREA (SPM_STORAGEAREA)
--两库中 TYPE 定义一致，不需要转义处理
--两库中 STATUS 定义一致，不需要转义处理
